...
public final class CadRenderer {
    ...
    private static void renderLayer(CadBakedModel model, CadMaterial material, String layer,
        net.minecraft.util.ResourceLocation texture, int colour) {
        boolean found = false;
        for (CadBakedModel.Quad q : model.quads) if (layer.equals(q.layer)) {
            found = true;
            break;
        }
        if (!found) return;
