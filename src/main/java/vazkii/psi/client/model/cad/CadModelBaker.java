...
public final class CadModelBaker {
    ...
    public static CadBakedModel bake(CadModel model) {
        ...
        for (CadModel.Element e : model.elements) for (Map.Entry<String, CadModel.Face> entry : e.faces.entrySet()) {
            float[] v = vertices(entry.getKey(), e.from, e.to);
            if (v == null) continue;
    ...
    private static float[] vertices(String d, float[] f, float[] t) {
        float x0 = f[0] / 16F, y0 = f[1] / 16F, z0 = f[2] / 16F, x1 = t[0] / 16F, y1 = t[1] / 16F, z1 = t[2] / 16F;
        ...
        if ("north".equals(d)) return new float[] { x0, y0, z0, x0, y1, z0, x1, y1, z0, x1, y0, z0 };
        if ("south".equals(d)) return new float[] { x1, y0, z1, x1, y1, z1, x0, y1, z1, x0, y0, z1 };
        if ("west".equals(d)) return new float[] { x0, y0, z1, x0, y1, z1, x0, y1, z0, x0, y0, z0 };
        if ("east".equals(d)) return new float[] { x1, y0, z0, x1, y1, z0, x1, y1, z1, x1, y0, z1 };
        if ("up".equals(d)) return new float[] { x0, y1, z0, x0, y1, z1, x1, y1, z1, x1, y1, z0 };
        if ("down".equals(d)) return new float[] { x0, y0, z1, x0, y0, z0, x1, y0, z0, x1, y0, z1 };
        return null;
    }
    ...
    private static void rotate(float[] p, int i, float[] origin, String axis, float degrees) {
        if (axis == null || degrees == 0) return;
        float ox = origin[0] / 16F, oy = origin[1] / 16F, oz = origin[2] / 16F, x = p[i] - ox, y = p[i + 1] - oy,
            z = p[i + 2] - oz;
    ...
    private static float[] normal(float[] v) {
        float ax = v[3] - v[0], ay = v[4] - v[1], az = v[5] - v[2], bx = v[6] - v[0], by = v[7] - v[1],
            bz = v[8] - v[2];
        float x = ay * bz - az * by, y = az * bx - ax * bz, z = ax * by - ay * bx,
            l = (float) Math.sqrt(x * x + y * y + z * z);
        return l == 0 ? new float[] { 0, 1, 0 } : new float[] { x / l, y / l, z / l };
    }
}
