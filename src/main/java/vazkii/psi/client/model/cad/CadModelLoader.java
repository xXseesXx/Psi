...
public final class CadModelLoader {
    ...
    public static CadModel load(ResourceLocation location) {
        try {
            ...
            for (Map.Entry<String, JsonElement> e : root.getAsJsonObject("textures")
                .entrySet())
                textures.put(
                    e.getKey(),
                    e.getValue()
            ...
            if (display != null) for (Map.Entry<String, JsonElement> e : display.entrySet()) {
                JsonObject v = e.getValue()
                    .getAsJsonObject();
                transforms.put(
                    e.getKey(),
                    new CadModel.Transform(
                        array(v, "rotation", 0F),
                        array(v, "translation", 0F),
                        array(v, "scale", 1F)));
            }
            ...
            for (JsonElement value : root.getAsJsonArray("elements")) {
                ...
                if (e.has("rotation")) {
                    ...
                    angle = r.get("angle")
                        .getAsFloat();
                    origin = array(r, "origin", 0F);
                }
                Map<String, CadModel.Face> faces = new LinkedHashMap<String, CadModel.Face>();
                for (Map.Entry<String, JsonElement> f : e.getAsJsonObject("faces")
                    .entrySet()) {
                    JsonObject x = f.getValue()
                        .getAsJsonObject();
                    faces.put(
                        f.getKey(),
                        new CadModel.Face(
                            ...
                            x.get("texture")
                                .getAsString(),
                            x.has("rotation") ? x.get("rotation")
                                .getAsInt() : 0,
                            x.has("cullface") ? x.get("cullface")
                                .getAsString() : null));
                }
                elements.add(
                    new CadModel.Element(
                        e.has("name") ? e.get("name")
                            .getAsString() : "",
                        array(e, "from", 0F),
                        array(e, "to", 0F),
                        origin,
            ...
            float[] size = array(root, "texture_size", 32F);
            return new CadModel(elements, textures, transforms, size[0], size[1]);
        } catch (Exception e) {
            throw new IllegalStateException("Unable to load CAD model " + location, e);
        }
    }
    ...
    private static float[] array(JsonObject o, String key, float fallback) {
        ...
        Arrays.fill(a, fallback);
        if (!o.has(key)) return a;
        JsonArray in = o.getAsJsonArray(key);
        for (int i = 0; i < in.size() && i < length; i++) a[i] = in.get(i)
            .getAsFloat();
        return a;
    }
}
