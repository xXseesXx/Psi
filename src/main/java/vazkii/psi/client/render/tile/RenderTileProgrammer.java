...
public final class RenderTileProgrammer extends TileEntitySpecialRenderer {
    private static final ResourceLocation PROGRAMMER_TEXTURE = new ResourceLocation("psi", "textures/gui/programmer.png");
    ...
    public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float partialTicks) {
        ...
        int facing = programmer.getWorldObj().getBlockMetadata(programmer.xCoord, programmer.yCoord, programmer.zCoord) & 3;
    ...
    private void drawPanel() {
        Minecraft.getMinecraft().getTextureManager().bindTexture(PROGRAMMER_TEXTURE);
    ...
    private void drawPieces(TileProgrammer programmer) {
        for (int gridX = 0; gridX < 9; gridX++) for (int gridY = 0; gridY < 9; gridY++) {
            ...
            PieceTextureAtlas.getInstance().drawPiece(piece.registryKey.toString(), x, y);
            if (piece instanceof PieceConstantNumber) {
                String value = ((PieceConstantNumber) piece).getDisplayValue();
                if (value.length() > 5) value = value.substring(0, 5);
                Minecraft.getMinecraft().fontRenderer.drawStringWithShadow(value, x + 9 - Minecraft.getMinecraft().fontRenderer.getStringWidth(value) / 2, y + 5, 0xFFFFFF);
            }
        }
    }
    ...
    private void drawName(TileProgrammer programmer) {
        ...
        Minecraft.getMinecraft().fontRenderer.drawString(programmer.spell.name == null ? "" : programmer.spell.name, 38, 164, 0xFFFFFF);
    }
        ...
        for (int x = 0; x < 9; x++) for (int y = 0; y < 9; y++) {
            ...
            for (java.util.Map.Entry<SpellParam<?>, SpellParam.Side> entry : piece.paramSides.entrySet()) {
                ...
                if (neighbor != null) {
                    ...
                    if (side.asInt() > side.getOpposite().asInt()) index += otherCount;
                    count += otherCount;
                }
