...
public class KeybindHandler {
    ...
    public static void init() {
        ClientRegistry.registerKeyBinding(PSI_MASTER);
    }

    @SubscribeEvent
    public void clientTick(TickEvent.ClientTickEvent event) {
        if (event.phase != TickEvent.Phase.END || !PSI_MASTER.isPressed()) return;
        Minecraft mc = Minecraft.getMinecraft();
        if (mc.currentScreen != null || mc.thePlayer == null) return;
        ItemStack held = mc.thePlayer.getHeldItem();
        if (held != null && (held.getItem() instanceof ItemCreativeCAD || held.getItem() instanceof ItemCAD))
            mc.displayGuiScreen(new GuiCADSelect(held));
    }
}
