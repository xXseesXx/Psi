...
public final class SharingHelper {
        ...
        new Thread("Psi Reddit Share") {
            ...
            public void run() {
                ...
                try {
                    ...
                    open(
                        "https://old.reddit.com/r/psispellcompendium/submit?title=" + URLEncoder.encode(title, "UTF-8")
                            + "&text="
    ...
    private static String upload(String title, String spellCode, String screenshot) {
        try {
            String form = "type=base64&image=" + URLEncoder.encode(screenshot, "UTF-8")
                ...
                + "&description="
                + URLEncoder.encode("Spell Code:\n\n" + spellCode, "UTF-8");
            HttpURLConnection connection = (HttpURLConnection) new URI("https://api.imgur.com/3/image").toURL()
                .openConnection();
            ...
            connection.getOutputStream()
                .write(form.getBytes("UTF-8"));
            InputStream input = connection.getResponseCode() / 100 == 2 ? connection.getInputStream()
            ...
            java.util.regex.Matcher id = java.util.regex.Pattern.compile("\\\"id\\\"\\s*:\\s*\\\"([^\\\"]+)\\\"")
                .matcher(json);
    ...
    public static String takeScreenshot() throws Exception {
        ...
        for (int y = 0; y < height; y++) for (int x = 0; x < width; x++) {
            int i = (x + width * y) * 4, r = pixels.get(i) & 255, g = pixels.get(i + 1) & 255,
                b = pixels.get(i + 2) & 255, a = pixels.get(i + 3) & 255;
    ...
    private static void open(String url) {
        try {
            if (url != null && Desktop.isDesktopSupported()) Desktop.getDesktop()
                .browse(new URI(url));
        } catch (Exception ignored) {}
    }
}
