...
/**
 * A label for displaying spell stat calculations as mathematical expressions.
 * Supports operator precedence and parenthesization for proper display.
 * 
 * 1.7.10 Backport: Removed modern text component handling, uses plain strings.
 */
public class StatLabel {
    ...
    public StatLabel pow(StatLabel other) {
        return append("^(").append(other)
            .append(")");
    }

    public StatLabel pow(double value) {
        return append("^(").append(formatDouble(value))
            .append(")");
    }
