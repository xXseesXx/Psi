...
public class TileCADAssembler extends TileEntity implements IInventory {
    ...
    @Override public ItemStack decrStackSize(int slot, int amount) {
        ...
        if (stack.stackSize <= amount) { inventory[slot] = null; changed(slot); return stack; }
        ItemStack result = stack.splitStack(amount); if (stack.stackSize == 0) inventory[slot] = null; changed(slot); return result;
    }
    @Override public ItemStack getStackInSlotOnClosing(int slot) { ItemStack stack = inventory[slot]; inventory[slot] = null; changed(slot); return stack; }
    ...
    @Override public boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        return slot >= SLOT_BULLET_START && slot < SLOT_OUTPUT && stack.getItem() instanceof ItemSpellBullet
    ...
    private void updateCraftResult() {
        ...
        if (assembly != null) inventory[SLOT_OUTPUT] = ItemCAD.createCAD(assembly, inventory[SLOT_CORE], inventory[SLOT_SOCKET], inventory[SLOT_BATTERY]);
    }

    /** Called by the output slot after a player takes the assembled CAD. */
    public void craftCAD() {
        inventory[SLOT_ASSEMBLY] = null; inventory[SLOT_CORE] = null; inventory[SLOT_SOCKET] = null; inventory[SLOT_BATTERY] = null;
        inventory[SLOT_OUTPUT] = null;
        markDirty();
    }
    ...
    public boolean isBulletSlotEnabled(int slot) {
        ...
        return cad.getItem() instanceof ItemCreativeCAD || cad.getItem() instanceof ItemCAD && slot < ItemCAD.getMagazineSize(cad);
    }
    private void loadMagazine() { for (int i = 0; i < MAGAZINE_SLOTS; i++) { ItemStack cad = inventory[SLOT_CAD]; inventory[SLOT_BULLET_START + i] = cad != null && isBulletSlotEnabled(i) ? (cad.getItem() instanceof ItemCreativeCAD ? ItemCreativeCAD.getBullet(cad, i) : ItemCAD.getBullet(cad, i)) : null; } }
    public void clearMagazineView() { for (int i = 0; i < MAGAZINE_SLOTS; i++) inventory[SLOT_BULLET_START + i] = null; markDirty(); }
    private void saveMagazineSlot(int slot) { if (inventory[SLOT_CAD] != null) { int bulletSlot = slot - SLOT_BULLET_START; if (inventory[SLOT_CAD].getItem() instanceof ItemCreativeCAD) ItemCreativeCAD.setBullet(inventory[SLOT_CAD], bulletSlot, inventory[slot]); else if (inventory[SLOT_CAD].getItem() instanceof ItemCAD) ItemCAD.setBullet(inventory[SLOT_CAD], bulletSlot, inventory[slot]); } }
