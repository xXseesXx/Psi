 ...
 * Psi-1.21.1/src/main/java/vazkii/psi/common/spell/operator/entity/PieceOperatorClosestToPoint.java:24
 * Modern: Entity.getX/Y/Z, MathHelper.pointDistanceSpace. GTNH: Entity.posX/Y/Z, same MathHelper.
 * Harder piece: requires EntityListWrapper + distance, but no World query — pure math, serviceable manual craft.
 */
...
public class PieceOperatorClosestToPoint extends PieceOperator {
    ...
    public static Entity closestToPoint(Vector3 position, Iterable<Entity> list) throws SpellRuntimeException {
        ...
        for (Entity e : list) {
            double dist = MathHelper.pointDistanceSpace(position.x, position.y, position.z, e.posX, e.posY, e.posZ);
            if (dist < closest) {
                closest = dist;
                closestEntity = e;
            }
