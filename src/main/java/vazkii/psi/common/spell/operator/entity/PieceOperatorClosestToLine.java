 ...
 * Psi-1.21.1/src/main/java/vazkii/psi/common/spell/operator/entity/PieceOperatorClosestToLine.java:23
 * Modern: Vec3 with x()/y()/z(), dot, length, cross. GTNH: Vec3 xCoord/yCoord/zCoord, dotProduct, lengthVector,
 * crossProduct, Vec3.createVectorHelper.
 */
...
public class PieceOperatorClosestToLine extends PieceOperator {
    ...
    public static Entity closestToLineSegment(Vector3 a, Vector3 b, Iterable<Entity> list)
        ...
        for (Entity e : list) {
            Vec3 pos = Vec3.createVectorHelper(e.posX, e.posY, e.posZ);
            double dot = diff.dotProduct(pos);
            double dist;
            if (dot <= minDot) {
                dist = pos.subtract(start)
                    .lengthVector();
            } else if (dot >= maxDot) {
                dist = pos.subtract(end)
                    .lengthVector();
            } else {
                dist = pos.subtract(start)
                    .crossProduct(diff)
