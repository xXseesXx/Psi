...
public class ItemCreativeCAD extends Item {
    ...
    public static void setBullet(ItemStack cad, int slot, ItemStack bullet) {
        if (cad.getTagCompound() == null) cad.setTagCompound(new NBTTagCompound());
    ...
    public static int getSelectedSlot(ItemStack cad) {
        return cad.hasTagCompound() ? Math.max(
            0,
            Math.min(
                MAGAZINE_SIZE - 1,
                cad.getTagCompound()
    ...
    public static void setSelectedSlot(ItemStack cad, int slot) {
        if (cad.getTagCompound() == null) cad.setTagCompound(new NBTTagCompound());
        cad.getTagCompound()
            .setInteger("selectedSlot", Math.max(0, Math.min(MAGAZINE_SIZE - 1, slot)));
    }

    @Override
    public ItemStack onItemRightClick(ItemStack cad, World world, EntityPlayer player) {
        ...
        player.addChatMessage(
            new ChatComponentText(EnumChatFormatting.RED + "[Psi] No programmed spell bullet selected"));
        return cad;
    }
}
